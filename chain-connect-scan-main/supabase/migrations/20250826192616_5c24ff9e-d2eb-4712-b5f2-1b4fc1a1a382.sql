...
-- Leaders can view contributions of their syndicat members
...
USING (member_id IN (
    ...
    WHERE sm.syndicat_id IN (SELECT id FROM public.unions WHERE leader_id = auth.uid())
));
...
USING (syndicat_id IN (
    SELECT sm.syndicat_id FROM public.syndicat_members sm 
...
USING (meeting_id IN (
    ...
    JOIN public.syndicat_members sm ON m.syndicat_id = sm.syndicat_id
    WHERE sm.user_id = auth.uid()
));
