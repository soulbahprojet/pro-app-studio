-- Fix the process_wallet_transfer function to use proper reference IDs instead of generating random UUIDs
...
BEGIN
  ...
  -- Create fee transaction for platform (if fee > 0) - use system wallet (null)
  IF p_fee > 0 THEN
    INSERT INTO transactions (
      wallet_id, type, amount, currency, status, description, reference_id
    ) VALUES (
